...
export default function SignUp(){
  ...
  async function sendLink(){
    if(!first || !email) return setNotice("First name and email required.");
  ...
  useEffect(() => {
    const sub = supabase.auth.onAuthStateChange(async (_e, sess) => {
      if (sess?.access_token) {
        const first = localStorage.getItem("pending_first_name");
        if (first) {
  ...
  return (
    <div className="max-w-md space-y-3">
      <input className="bg-white/10 px-3 py-2 rounded w-full" placeholder="First name" value={first} onChange={e=>setFirst(e.target.value)} />
